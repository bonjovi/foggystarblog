...
<div id="primary" class="content-area">
	<main id="main" class="site-main" role="main">
		...
		if ( is_date() && in_array( 'date_template', $archive_list ) ) { //phpcs:ignore
			...
			$array                     = array_keys( $archive_list, 'date_template' ); //phpcs:ignore
			$archive_id                = $array[0];
			...
			$array                     = array_keys( $archive_list, 'search_template' ); //phpcs:ignore
			$archive_id                = $array[0];
			$bdp_theme                 = apply_filters( 'bdp_filter_template', $bdp_settings['template_name'] );
			$bdp_template_name_changed = get_option( 'bdp_template_name_changed', 1 );
			if ( 1 == $bdp_template_name_changed ) { //phpcs:ignore
		...
		global $wp_query;
		$paged     = Bdp_Utility::paged(); //phpcs:ignore
		...
		<div class="blog_template bdp_wrapper <?php echo esc_attr( $bdp_theme ); ?>_cover bdp_archive <?php echo esc_attr( $bdp_theme ) . ' layout_id_' . esc_attr( $archive_id ); ?> ">
			...
			if ( $bdp_is_author && in_array( 'author_template', $archive_list ) ) { //phpcs:ignore
			...
			if ( $loop->have_posts() ) {
				while ( have_posts() ) :
					...
					echo apply_filters( 'bdads_do_show_ads', '', $bdp_settings, $alter, $bdp_theme, $paged ); //phpcs:ignore
					$alter ++;
					// End of the loop.
				endwhile;
			} else {
			...
			if ( ! in_array( $bdp_theme, $slider_array ) && ( isset( $bdp_settings['pagination_type'] ) && 'no_pagination' !== $bdp_settings['pagination_type'] ) ) { //phpcs:ignore
				if ( $max_num_pages > 1 && ( isset( $bdp_settings['pagination_type'] ) && 'load_more_btn' === $bdp_settings['pagination_type'] ) ) {
					...
					if ( '' === $is_loadmore_btn ) {
						...
						echo ( isset( $bdp_settings['loadmore_button_text'] ) && '' != $bdp_settings['loadmore_button_text'] ) ? $bdp_settings['loadmore_button_text'] : esc_html__( 'Load More', 'blog-designer-pro' ); //phpcs:ignore
						echo '</a>';
						echo '</div>';
					}
