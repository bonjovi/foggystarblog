...
if ( ! function_exists( 'bdp_archive_neaty_block_template' ) ) {
	...
	function bdp_archive_neaty_block_template( $bdp_settings ) {
			...
			if ( ! empty( $thumbnail ) ) {
				...
				if ( 1 == $display_author ) { //phpcs:ignore
					...
					<div class="neaty-block-avatar">
						<?php echo get_avatar( get_the_author_meta( 'ID' ), 70 ); //phpcs:ignore ?>
					</div>
				<?php } ?>
					<div class="bdp-comments-box">
						<ul>
							...
							if ( 1 == $display_postlike ) { //phpcs:ignore
								echo '<li>' . do_shortcode( '[likebtn_shortcode]' ) . '</li>'; //phpcs:ignore
							}
		...
		if ( 1 == $display_author || 1 == $display_comment_count || 1 == $display_postlike ) { //phpcs:ignore
			if ( 1 == $display_author ) { //phpcs:ignore
